...
class MultiSelectField extends ChoicesField{
	function input_html(){
		ob_start();
		?>
		<select multiple name="<?=htmlentities($this->id)?>" id="<?=htmlentities($this->id)?>">
			<?php foreach($this->choices as $key=>$value):?>
			<option<?php if($this->value == $value):?> selected="selected"<?php endif;?> value="<?=htmlentities($value)?>"><?=htmlentities($key)?></option>
			<?php endforeach;?>
		</select>
...
function sc_object_list($attrs, $options = array()){
	...
	if (!is_numeric($params['posts_per_page'])){
	...
	if (null == ($class = get_custom_post_type($params['post_type']))){
	...
	$taxonomies = array_intersect(get_taxonomies(), array_keys($params));
	...
	# perform query
	$query_array = array_merge($params, array(
 ...
 * @author Jared Lang
 **/
function show_meta_boxes($post, $meta_box=null){
	#Register custom post types metaboxes
	$custom_post_type = get_custom_post_type(post_type($post));
	$custom_post_type = new $custom_post_type;

	if ($meta_box['args']) {
		$metabox_id = $meta_box['args']['metabox_id'];
		if ($metabox_id) {
			$meta_box = $custom_post_type->get_metabox($metabox_id);
		}
	}
	else {
		$meta_box = $custom_post_type->metabox();
...
function save_default( $post_id, $field ){
	...
	if ( $post_type ) {
		...
		// loop through clonable fields and remove items if they are all empty
		if ( $post_type->options( 'cloneable_fields' ) && is_array( $new ) ) {
			$new_temp = $new;
			foreach ( $new as $index => $array ) {
				if( !array_filter( $array ) ) {
					unset( $new_temp[$index] );
				}
			}
			$new = array_values( $new_temp );
		}
	}
...
function _save_meta_data( $post_id, $meta_box ) {
	...
	if ( $post_type ) {
		...
		if ( $post_type->options( 'cloneable_fields' ) ) {
			$nonce_fields = $post_type->options( 'cloneable_field_nonces' );
			if ( $nonce_fields ) {
	...
	<?php foreach($meta_box['fields'] as $field):
		...
		<tr>
			...
			<?php break; case 'multiselect':?>
				<select multiple name="<?=$field['id']?>[]" id="<?=$field['id']?>">
					<option value=""><?=($field['default']) ? $field['default'] : '--'?></option>
				<?php foreach ($field['options'] as $k=>$v):?>
					<option <?php if (in_array($v, $current_value)) { ?>selected<?php } ?> value="<?=$v?>"><?=$k?></option>
				<?php endforeach;?>
				</select>
